...
async function downloadIfNeeded(url: string, outPath: string, force: boolean) {
  ...
  const res = await fetch(url, {
    headers: {
      Accept: 'image/*',
      'User-Agent': 'sopla-el-cartucho-jueves-de-quack/assets-generator',
    },
  });
...
async function main() {
  ...
  const sessions = await readJsonFile<SessionsFile>(SESSIONS_PATH);
  const existingAssets = await readAssetsFileOrEmpty();

  const existingBySessionId = new Map(existingAssets.assets.map((a) => [a.sessionId, a] as const));
  ...
  for (const session of sessions.sessions) {
    ...
    // Use thumbnail as avatar source for YouTube sessions
    const avatarUrl = session.speakers?.[0]?.avatarUrl || session.thumbnailUrl;
    if (!avatarUrl) {
      outAssets.push({ sessionId: session.id, cover: existing?.cover });
      continue;
    }
    ...
    const baseSlug = slugify(speakerName || session.id) || session.id;
    ...
    const speakerSlug = !priorUrl
      ...
      : priorUrl === avatarUrl
        ? baseSlug
        : `${baseSlug}-${shortHash(avatarUrl)}`;

    // Reserve both the base slug and (if needed) the disambiguated slug.
    if (!priorUrl) slugToUrl.set(baseSlug, avatarUrl);
    slugToUrl.set(speakerSlug, avatarUrl);

    const ext = safeExtFromUrl(avatarUrl);
    ...
    try {
      const dl = await downloadIfNeeded(avatarUrl, originalFsPath, force);
      ...
      process.stderr.write(`WARN: ${session.id}: failed to generate avatar (${String(err)})\n`);
      outAssets.push({ sessionId: session.id, cover: existing?.cover });
    }
  }
