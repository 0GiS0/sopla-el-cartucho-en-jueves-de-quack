...
const SESSIONS_PATH = path.join('src', 'data', 'sessions.json');
...
const SESSION_TYPES_TO_RENDER = new Set(['episode'] as const);
...
function pickPalette(sessionId: string) {
  ...
  const h = stableHashHex(sessionId);
...
function buildBackgroundSvg({ sessionId }: { sessionId: string }) {
  const { bg, a1, a2, a3 } = pickPalette(sessionId);
  const h = stableHashHex(sessionId);
...
function buildOverlaySvg({
  ...
  const { a2, ink } = pickPalette(sessionId);
...
<svg xmlns="http://www.w3.org/2000/svg" width="${COVER_W}" height="${COVER_H}" viewBox="0 0 ${COVER_W} ${COVER_H}">
  ...
  <!-- corner micro-label -->
  <text x="${pad}" y="${COVER_H - 18}" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="12" fill="${a2}" opacity="0.85">${escapeXml(sessionId)}</text>
</svg>`;
}
...
async function generateCover({
  ...
  const bgSvg = buildBackgroundSvg({ sessionId });
  const overlaySvg = buildOverlaySvg({ sessionId, title, speaker });
...
async function main() {
  ...
  const sessionsFile = await readJsonFile<SessionsFile>(SESSIONS_PATH);
  const assetsFile = await readJsonFile<AssetsFile>(ASSETS_PATH);

  const existingBySessionId = new Map(assetsFile.assets.map((a) => [a.sessionId, a] as const));
  ...
  for (const session of sessionsFile.sessions) {
    const existing = existingBySessionId.get(session.id) ?? { sessionId: session.id };

    if (!SESSION_TYPES_TO_RENDER.has(session.type as any)) {
      outAssets.push(existing);
      continue;
    }

    const coverWebPath = `${COVERS_DIR_WEB}/${session.id}.png`;
    const coverFsPath = path.join(process.cwd(), COVERS_DIR_FS, `${session.id}.png`);
